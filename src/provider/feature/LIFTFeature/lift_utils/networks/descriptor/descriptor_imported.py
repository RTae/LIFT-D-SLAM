from lasagne.layers import (Conv2DLayer, ExpressionLayer, FlattenLayer, MaxPool2DLayer, NonlinearityLayer)
from src.provider.feature.LIFTFeature.lift_utils.lasagne_tools import LPPool2DLayer, SubtractiveNormalization2DLayer
...
def build(myNet, idxSiam, verbose=True):
    ...
    # Load model
    fn = '%s/%s' % (myNet.config.descriptor_export_folder, myNet.config.descriptor_model)
    print(fn)
    model_dict = loadh5(fn)
